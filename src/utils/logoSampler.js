/**
 * Samples pixels from the Aranea SVG to build a particle formation.
 *
 * Rendering strategy:
 *   - All colors forced to white, transparent background (no black fill).
 *   - Pixels accepted only when alpha > 200 — this skips anti-aliased fringe
 *     pixels (alpha 20-150) while keeping solid stroke/fill pixels (alpha 255).
 *   - Fills are KEPT (not stripped) so the thick hexagonal outer border ring,
 *     which is a filled compound path, contributes particles correctly.
 *   - Only fill:none inner paths (.c3) have no fill area — their interiors
 *     remain transparent and are not sampled, giving the hollow center.
 *   - Fisher-Yates shuffle ensures even spatial coverage before slicing.
 */
...
// Make all colors white; keep fills intact (the outer border ring is a filled path)
function prepareWhiteSvg(raw) {
  return raw
    // Remove any background colour on the svg element itself
    .replace(/(<svg\b[^>]*?)\s+style="[^"]*"/i, '$1')
    // CSS class rules: convert fills to white (keep fill:none untouched)
    .replace(/fill\s*:\s*(?!none)[^;}"]+/g,  'fill:white')
    // CSS class rules: convert strokes to white
    .replace(/stroke\s*:\s*[^;}"]+/g,        'stroke:white')
    // Inline fill attributes: white (keep fill="none" untouched)
    .replace(/fill="(?!none)[^"]*"/g,        'fill="white"')
    // Inline stroke attributes: white
    .replace(/stroke="(?!none)[^"]*"/g,      'stroke="white"')
}

export async function sampleLogoPositions(svgUrl, particleCount, W, H) {
  const SZ              = 1024   // offscreen canvas resolution
  const ALPHA_THRESHOLD = 200    // skips anti-aliased semi-transparent fringe pixels
  const vW              = W * 2  // actual viewport width

  // Responsive width: ~50% on mobile, max 350px on desktop
  const LOGO_WIDTH = vW < 768
    ? Math.round(vW * 0.50)
    : Math.min(350, vW - 48)

  try {
    // 1. Load SVG — fall back to inline copy on any network error
    let svgSource = prepareWhiteSvg(FALLBACK_SVG)
    try {
      const res = await fetch(svgUrl, { mode: 'cors' })
      if (res.ok) svgSource = prepareWhiteSvg(await res.text())
    } catch (_) { /* use fallback */ }

    // 2. Blob URL from prepared SVG
    const blob    = new Blob([svgSource], { type: 'image/svg+xml' })
    const blobUrl = URL.createObjectURL(blob)

    // 3. Offscreen canvas — transparent background (NO fillRect)
    let canvas, ctx
    ...
    // 4. Wait for full SVG load, then draw — no background, alpha channel intact
    await new Promise((resolve, reject) => {
      const img = new Image()
      img.onload = () => { ctx.clearRect(0, 0, SZ, SZ); ctx.drawImage(img, 0, 0, SZ, SZ); resolve() }
      img.onerror = reject
      img.src = blobUrl
    })
    ...
    // 5. Collect fully-opaque pixels only (alpha > ALPHA_THRESHOLD)
    //    — skips anti-aliased fringe AND the interior of fill:none paths
    const pixels = ctx.getImageData(0, 0, SZ, SZ).data
    const cands  = []
    for (let y = 0; y < SZ; y++) {
      for (let x = 0; x < SZ; x++) {
        if (pixels[(y * SZ + x) * 4 + 3] > ALPHA_THRESHOLD) cands.push({ x, y })
      }
    }

    if (cands.length < 50) {
      console.warn('[logoSampler] Too few opaque pixels:', cands.length)
      return null
    }

    // 6. Fisher-Yates shuffle — even spatial coverage across the full logo
    ...
    //    Portrait SVG (116.48 × 133.78) letterboxes horizontally in the square canvas:
    //    content fills full height, centred left-right with equal margins.
    const svgAR   = SVG_W / SVG_H           // ≈ 0.871
    const contentW = SZ * svgAR             // canvas px occupied by SVG content
    const xOffset  = (SZ - contentW) / 2   // left letterbox margin

    const sx = LOGO_WIDTH / contentW        // world px per canvas px (uniform scale)
    const sy = sx

    // Logo centre at 35 svh from viewport top = H × 0.30 above world centre
    const yOff = H * 0.30
    ...
    console.log(`[logoSampler] ${cands.length} opaque px → ${sampled.length} particles, logo ${LOGO_WIDTH}px wide`)
